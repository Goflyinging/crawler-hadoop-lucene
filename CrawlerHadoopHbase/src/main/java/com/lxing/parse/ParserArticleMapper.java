 ...
 */
public class ParserArticleMapper extends
        ...
        if (url.matches(CSDNarticleRegex)) {
            for (Cell cell : values.rawCells()) {
                if ("document".equals(Bytes.toString(CellUtil.cloneQualifier(cell)))
                    ...
                    context.write(new ImmutableBytesWritable(articleTable.getBytes()),
                                  put);
                    context.write(new ImmutableBytesWritable(cacheArticleTable.getBytes()),
                                  put);
                }
            }
        }
