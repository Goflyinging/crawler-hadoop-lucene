...
/**
 * Created by lxing on 2017/4/11. url 去重 将状态为1的 即刚抓取的网页的outlinks 和原来的网页比对并去重
 * 如果为false 即表明没有抓取过 存入表中等待下次抓取
 */
public class OptimizerDriver extends Configured implements Tool {
    
    public static Logger logger =
                                LoggerFactory.getLogger(OptimizerDriver.class);
    ...
    public int run(String[] args) throws ClassNotFoundException,
        ...
        TableMapReduceUtil.initTableMapperJob(urlTableName,
                                              ...
                                              job);
        TableMapReduceUtil.initTableReducerJob(urlTableName,
                                               OptimizerReducer.class,
                                               job);
